...
void    draw_replay(t_all **all, t_img *im, int k1, int k2)
{
   t_draw draw;
    ...
    while (draw.start[X] < draw.end[X])
	{
		...
		draw.start[Y] = (*all)->r[Y] / 2 - draw.h / 2;
        draw.end[Y] = draw.start[Y] + draw.h;
		while (draw.start[Y] < draw.end[Y])
		{
            draw.pix[Y] = draw.index[Y] * im->dim[Y] / draw.h;
		    if (im->data[(int)(im->dim[X] * draw.pix[Y] + draw.pix[X])] != -16777216)
			    (*all)->img.data[draw.start[X] + (*all)->r[X] * draw.start[Y]] = im->data[(int)(im->dim[X] * draw.pix[Y] + draw.pix[X])];
		    draw.start[Y]++;
            draw.index[Y]++;
		}
        draw.index[X]++;
