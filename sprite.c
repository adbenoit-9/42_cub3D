...
void	put_sprite(t_all **all)
{
	...
	(*all)->sp.endian = 0;
	(*all)->sp.ptr = mlx_xpm_file_to_image((*all)->mlx, (*all)->info[S], &(*all)->sp.dim[X], &(*all)->sp.dim[Y]);
...
void	raycast_sprite(t_all **all)
{
	...
	while (i < (*all)->sp.count)
	{
		...
		//printf("startX = %d screenX = %d\n", start[X], (*all)->sp.screen);
		while (start[X] < end[X])
		{
			...
			while (start[Y] < end[Y] && transf[Y] > 0 && start[X] > 0 && start[X] < (*all)->r[X] && transf[Y] < (*all)->wall.dist[start[X]])
			{
				...
				//printf("tex = (%d, %d)\n", tex[X], tex[Y]);
				if ((*all)->sp.data[(int)((*all)->sp.dim[X] * tex[Y] + tex[X])] != ft_rgb(0, 0, 0))
